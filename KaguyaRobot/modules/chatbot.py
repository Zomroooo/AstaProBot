...
from KaguyaRobot import BOT_USERNAME as bu
from KaguyaRobot import BOT_ID, pbot, arq
from KaguyaRobot.ex_plugins.chatbot import add_chat, get_session, remove_chat
from KaguyaRobot.utils.pluginhelper import admins_only, edit_or_reply
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace("@NexusXSupport", "Kaguyabot.ai")
        ...
        test = test.replace(
            "Hello there I am Kaguya...nice to meet u",
        ...
        response = response.replace("Aco", "Kaguya")
        response = response.replace("aco", "Kaguya")
        response = response.replace("Luna", "Kaguya")
        response = response.replace("luna", "Kaguya")
        ...
        response = response.replace("Kaguyabot.ai", "@NexusXSupport")
        ...
        test = test.replace("@machinaxsupport", "Kaguyabot.ai")
        ...
        response = response.replace("Kaguyabot.ai", "@machinaxsupport")
...
@pbot.on_message(filters.regex("Kaguya|Kaguya|robot|KAGUYA|vain") & ~filters.bot & ~filters.via_bot  & ~filters.forwarded & ~filters.reply & ~filters.channel & ~filters.edited)
async def inuka(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
